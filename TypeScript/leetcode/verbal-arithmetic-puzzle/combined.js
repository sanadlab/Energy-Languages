...
function isSolvable(words, result) {
    ...
    const assigned = new Array(128).fill(-1);
    const usedDigit = new Array(10).fill(false);
    const leading = new Array(128).fill(false);
    for (const w of words) {
        if (w.length > maxLen)
            return false;
        if (w.length > 1)
            leading[w.charCodeAt(0)] = true;
    }
    if (result.length > 1)
        leading[result.charCodeAt(0)] = true;
    const solve = (col, row, carry) => {
        ...
        for (const w of words) {
            if (col < w.length)
                sum += assigned[w.charCodeAt(w.length - 1 - col)];
        }
        const digit = sum % 10;
...
try {
    _lc_test_result = eval('new Solution().isSolvable(["a","b","c"], "abcde")');
}
catch (_e) {
    _lc_test_result = eval('isSolvable(["a","b","c"], "abcde")');
}
