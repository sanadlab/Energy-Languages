...
function hitBricks(grid, hits) {
    ...
    const total = m * n;
    const top = total;
    const parent = new Array(total + 1);
    const sz = new Array(total + 1);
    ...
    for (let i = hits.length - 1; i >= 0; i--) {
        ...
        if (!inb(r, c))
            continue;
        if (!(Array.isArray(grid[r]) && grid[r][c] === 1))
            continue;
...
try {
    _lc_test_result = eval('new Solution().hitBricks([[1,2],[3,4]], [[1,2],[3,4]])');
}
catch (_e) {
    _lc_test_result = eval('hitBricks([[1,2],[3,4]], [[1,2],[3,4]])');
}
