function minAreaRect(points: number[][]): number {
    ...
    const enc = (x: number, y: number): number => x * 50000 + y;
...
try { _lc_test_result = eval('new Solution().minAreaRect([[1,2],[3,4]])'); }
catch (_e) { _lc_test_result = eval('minAreaRect([[1,2],[3,4]])'); }
