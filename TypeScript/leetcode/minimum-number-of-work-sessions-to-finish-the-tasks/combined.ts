function minSessions(tasks: number[], sessionTime: number): number {
    ...
    const used: number[] = new Array(1 << n).fill(0);
    sessions[0] = 1;
...
try { _lc_test_result = eval('new Solution().minSessions([1,2,3,4,5], 20)'); }
catch (_e) { _lc_test_result = eval('minSessions([1,2,3,4,5], 20)'); }
